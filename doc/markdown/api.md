...
- <a name="wf_secret"></a>`wf:secret(Key, Default) -> Val`

  Retrieve the secret associated with the provided `Key`. The default behavior
  relies on the [nitro_secret](https://github.com/nitrogen/nitro_secret)
  application. See [Secrets Management] for more details. If no value is found,
  then it returns, `Default`.

- <a name="wf_secret"></a>`wf:secret(Key) -> Val`

  Same as `wf:secret(Key, undefined)`
...
- `?WF_IF(Term, IfTrue, IfFalse)`
  ...
  This is a ternary `if`. Basically, it's just a convenience macro that roughly
  simulates this case statement:
