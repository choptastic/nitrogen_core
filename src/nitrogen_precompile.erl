...
inc() ->
    V = erlang:get(precompiled_count),
    erlang:put(precompiled_count, V+1).
...
parse_transform(Forms, _Options) ->
    ...
    erlang:put(precompiled_count, 0),
    ...
    Count = erlang:get(precompiled_count),
    io:format("Precompiled ~p elements.~n", [Count]),
    NewForms.
...
do_function({function, Line, Function, Arity, Clauses}) ->
    {function, Line, Function, Arity, do_clauses(Clauses)};
do_function(Form) ->
    Form.
    
do_clauses(Clauses) ->
    [do_clause(Clause) || Clause <- Clauses].

do_clause({clause, Line, Arg1, Arg2, Body}) ->
    {clause, Line, Arg1, Arg2, do_body_forms(Body)}.

do_body_forms(Forms) when is_list(Forms) ->
    [do_body_forms(F) || F <- Forms];
do_body_forms(Form = {record, _Line, RecordName, _Fields}) ->
    maybe_dispatch_to_module(RecordName, Form);
do_body_forms(_Form = {cons, Line, Head, Tail}) ->
    {cons, Line, do_body_forms(Head), do_body_forms(Tail)};
do_body_forms(_Form = {'case', Line, Cond, Clauses}) ->
    {'case', Line, Cond, do_clauses(Clauses)};
do_body_forms(_Form = {call, Line, Fun, Args}) ->
    {call, Line, Fun, do_body_forms(Args)};
do_body_forms(_Form = {'fun', Line, {clauses, Clauses}}) ->
    {'fun', Line, {clauses, do_clauses(Clauses)}};
do_body_forms(Form) ->
