...
-spec render_element(#grid{}) -> body().
render_element(#grid{type=clear}) ->
    "<div class='clear'></div>\n";
...
is_grid_body(Body) ->
    F = fun(X) ->
        is_tuple(X)
            andalso size(X) > 3 
            andalso lists:member(element(#grid.module, X), [element_grid, ?MODULE])
    end,
